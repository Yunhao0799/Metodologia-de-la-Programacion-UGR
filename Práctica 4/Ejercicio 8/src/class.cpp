...
char Menu::get_Title(int pos)const{
    return titulo[pos];
}

int Menu::title_size()const{
    int tam = 0, i = 0;
    while(titulo[i] != '\0'){
        tam++;
        i++;
...
char Menu::getpos(int i, int j) const {
    return opc[i][j];
}
...
Menu &Menu::operator= (const Menu &otra){
    //copiamos el titulo
    int tam_title = otra.title_size();
    titulo = new char[tam_title];
    int i, j;
