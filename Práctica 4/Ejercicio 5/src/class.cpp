...
double MatrizDispersa::valor(int posx, int posy) const{
    ...
    if(posx < nfilas && posy < ncolumnas){
        for(int i = 0; !valor && i < numeroValores; i++){
...
MatrizDispersa MatrizDispersa::operator+(const MatrizDispersa &otra){
    ...
    for(i = 0; i < numeroValores; i++){
        ...
        if(repe){
            ...
            aux = new Valor[tam_sal + 1];
            for(int h = 0; h < tam_sal; h++)
                aux[h] = salida[h];
            aux[tam_sal] = valores[i];
            salida = aux;
            delete [] aux;
            tam_sal++;
        }
    ...
    MatrizDispersa out(salida, tam_sal);
...
MatrizDispersa MatrizDispersa::operator+(const MatrizDispersa &otra){
	...
	int tam_sal = getOcupados();

	for(int i = 0; i < otra.getFilas(); i++)
		for(int j = 0; j < otra.getColumnas(); j++)
			if (otra.valor(i,j) && !valor(i,j))
				tam_sal++;

	salida = new Valor[tam_sal];

	for(int i = 0; i < getOcupados(); i++)
		salida[i] = getElemento(i);

	int i, j, aux = getOcupados();
	bool repetido;
	
	for(i = 0; i < otra.getOcupados(); i++){
		repetido = false;
		
		for(j = 0; !repetido && j < getOcupados(); j++)
			if(getElemento(j).fila == otra.getElemento(i).fila && getElemento(j).columna == otra.getElemento(i).columna)
				repetido = true;
				
		if(repetido)
			salida[j-1].valor += otra.getElemento(i).valor;
		else
			salida[aux++] = otra.getElemento(i);
	}

    MatrizDispersa suma(salida, tam_sal);
